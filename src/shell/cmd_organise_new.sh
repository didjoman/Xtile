...
if [ $nbWindows -gt 0 ]; then
	...
	if [ $nbWindows -gt 0 ]; then
	    ...
	    $(do_move_and_resize $activeWin $(( $(get_util	_dimension_for_screen width ) / 8 ))  $(( $(get_util_dimension_for_screen height) / 8 )) $(( $(get_util_dimension_for_screen width) * 3 / 4 )) $(( $(get_util_dimension_for_screen height) * 3 / 4 ))  )&
	...
	for i in $windows; do
	    ...
	    $(do_move_and_resize $i $posX 0 $(( $(get_util_dimension_for_screen width) / $nbWindows )) $(get_util_dimension_for_screen height))&
	...
	windows=$(echo $windows | sed -n "s/\(.*\)$activeWin\(.*\)/$activeWin \1\2/p" | sed "s/  / /")
		...
		for i in $windows; do
			...
			$(do_move_and_resize $i 0 $posY $(get_util_dimension_for_screen width) $(( $(get_util_dimension_for_screen height) / $nbWindows )) )&
