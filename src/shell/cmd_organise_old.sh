...
if [ $nbWindows -gt 0 ]; then
	...
	for i in 1 2 3 4 ;do
		...
		for win in $windows; do    
		    ...
		    (
			$(do_move_in_real_space $win both $posX $posY )&
			$(do_resize_with_real_dimension $win both $(( $(get_util_dimension_for_screen width) / $nbCol)) $(( $(get_util_dimension_for_screen height) / $nbLine)) )&
			wait
		    )&
	...
	for i in 1 2 3; do
	    $(do_move_in_real_space $activeWin both $(( $(get_util_dimension_for_screen width ) / 8 ))  $(( $(get_util_dimension_for_screen height) / 8 )) )&
	    $(do_resize_with_real_dimension $activeWin both $(( $(get_util_dimension_for_screen width) * 3 / 4 )) $(( $(get_util_dimension_for_screen height) * 3 / 4 )) )&
	    wait
	done
	...
	for ind in 1 2 3 4; do
		...
		for i in $windows; do
			...
			$(do_move_in_real_space $i both $posX 0)&
			$(do_resize_with_real_dimension $i both $(( $(get_util_dimension_for_screen width) / $nbWindows )) $(get_util_dimension_for_screen height) )&
			wait
		    )&
		    winNum=$(( $winNum + 1 ))
	...
	for ind in 1 2; do
		...
		for i in $windows; do
			...
			$(do_move_in_real_space $i both 0 $posY)&
			$(do_resize_with_real_dimension $i both $(get_util_dimension_for_screen width) $(( $(get_util_dimension_for_screen height) / $nbWindows )) )&
			wait
		    )&
		    winNum=$(( $winNum + 1 ))
	...
	for i in 1 2;do
	    $(do_move_in_real_space $activeWin both 0 0 )&
	    $(do_resize_with_real_dimension $activeWin both $(( $(get_util_dimension_for_screen width) / 2 )) $(get_util_dimension_for_screen height) )&
	done
	...
	if [ $nbWindows -gt 0 ]; then
		...
		(
			...
			(
			    $(do_move_in_real_space $win both $posX $posY )&
			    $(do_resize_with_real_dimension $win both $(( $(get_util_dimension_for_screen width) / 2)) $(( $(get_util_dimension_for_screen height) / $nbWindows)) )&
			    wait
			)&
	...
	for i in 1 2;do
	    (
	    $(do_move_in_real_space $activeWin both 0 0 )&
	    $(do_resize_with_real_dimension $activeWin both $(get_util_dimension_for_screen width) $(( $(get_util_dimension_for_screen height) / 2)) )&
	    ) &
	done;
	...
	if [ $nbWindows -gt 0 ]; then
		...
		(
			...
			(
			    $(do_move_in_real_space $win both $posX $posY )&
			    $(do_resize_with_real_dimension $win both $(( $(get_util_dimension_for_screen width) / $nbWindows)) $(( $(get_util_dimension_for_screen height) / 2)) )&
			    wait
			)&
